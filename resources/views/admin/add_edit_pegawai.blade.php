...
<div class="container">
    <div class="col-md-12">
        <div class="form-appl">
            <h3>{{ $title }}</h3>
            <form class="form1" method="post" action="@if (isset($edit->id)) {{ route('pegawai.update', ['id' => $edit->id]) }}@else{{ route('pegawai.store') }} @endif" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group col-md-12 mb-3">
                    <label for="">Your Jabatan</label>
                    <input class="form-control" type="text" name="jabatan" placeholder="Enter Your Jabatan" value="@if (isset($edit->id)) {{ $edit->jabatan }}@else {{ old('jabatan') }} @endif">
                    @error('jabatan')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <a class="btn btn-danger" href="{{ route('pegawai.index') }}">Cancel</a>
            </form>
        </div>
    </div>
</div>
